...
void PetriScene::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    ...
    if (currentTool == PlaceTool) {
        ...
        while (collision) {
            ...
            for (Place* p : places) {
                // If the name is taken, increment and check the place list again
        ...
        while (collision) {
            ...
            for (Transition* t : transitions) {
                // If the name is taken, increment and check the transition list again
        ...
        // If clicked on empty space, ignore
        if (!clicked) {
            return;
        }

        if (!arcSource) {
            // If first click, save source
            arcSource = clicked;
        } 
        else {
            if (clicked != arcSource) {
                ...
                // Only create an arc if the source and destination are of different types
                if ((sourceIsPlace && destIsTransition) || (sourceIsTransition && destIsPlace)) {
                    // Check if an arc already exists between this place and transition
                    ...
                    // If there is not arc connecting them, create a new arc
                    if (!alreadyConnected) {
...
void PetriScene::clearNet() {
    ...
    // Clean each transition and its arcs
    for (Transition* transitionPtr : transitions) {
        for (Arc* arc : transitionPtr->inputArcs) {
            delete arc;
        }

        for (Arc* arc : transitionPtr->outputArcs) {
            delete arc;
        }
    ...
    // Just to be sure clear the transitions vector
    transitions.clear();
    ...
    // Just to be sure clear the places vector
    places.clear();
}
    ...
    if (clickedArc) {
        ...
        if (srcTrans) {
            srcTrans->outputArcs.erase(
                std::remove(srcTrans->outputArcs.begin(), srcTrans->outputArcs.end(), clickedArc),
        ...
        if (destTrans) {
            destTrans->inputArcs.erase(
                std::remove(destTrans->inputArcs.begin(), destTrans->inputArcs.end(), clickedArc),
        ...
        foreach (Arc* arc, arcsToDelete) {
            ...
            if (srcTrans) {
                srcTrans->outputArcs.erase(
                    std::remove(srcTrans->outputArcs.begin(), srcTrans->outputArcs.end(), arc),
                    srcTrans->outputArcs.end()
                );
            }
            ...
            if (destTrans) {
                destTrans->inputArcs.erase(
                    std::remove(destTrans->inputArcs.begin(), destTrans->inputArcs.end(), arc),
                    destTrans->inputArcs.end()
                );
            }
        ...
        if (clickedPlace) {
            places.erase(std::remove(places.begin(), places.end(), clickedPlace), places.end());
        }

        if (clickedTrans) {
            transitions.erase(std::remove(transitions.begin(), transitions.end(), clickedTrans), transitions.end());
        }
...
void PetriScene::mouseMoveEvent(QGraphicsSceneMouseEvent* event) {
    if (currentTool == DeleteTool && event->buttons() & (Qt::LeftButton | Qt::RightButton)) {
        deleteItemAt(event->scenePos());
        updateFireability();
        return;
    }
...
void PetriScene::fireTransition(Transition* t) {
    ...
    for (Arc* arc : t->inputArcs) {
        ...
        if (place) {
            ...
            log("TOKENS: " + place->name + " " + QString::number(oldTokens) + "->" 
                + QString::number(place->tokens), 2);
        }
    }
...
void PetriScene::stabilize() {
    ...
    // Fire all immediate transitions until stable
    while (fired) {
        fired = false;
    ...
    // Schedule timers for timed transitions which are fireable
    scheduleTimers();
}
...
void PetriScene::scheduleTimers() {
    for (Transition* t : transitions) {
        ...
        // Skip immediate transitions
        if (t->isImmediate()) {
            continue;
        }

        // Skip non-fireable transitions
        if (!isFireable(t)) {
            continue;
        }
...
void PetriScene::onTimerExpired(Transition* t) {
    ...
    // Check if the transition is still fireable at the moment of expiry
    if (!isFireable(t)) {
...
void PetriScene::postEvent(const QString& name) {
    // Fire immediate event-guarded transitions for this event
    for (Transition* t : transitions) {
        if (t->eventName != name) {
            continue;
        }
    ...
    // Schedule timed event-guarded transitions for this event
    for (Transition* t : transitions) {
        if (t->eventName != name) {
            continue;
        }
    ...
    // After event-guarded firings, re-stabilize the free transitions
    stabilize();
}
...
void PetriScene::rebuildCflatEnvironment() {
    ...
    for (auto& v : variables) {
        if (v.type == "float") {
            ...
            code += "int " + v.name.toStdString() + " = " 
                    + QString::number(v.initialValue.toString().at(0).unicode()).toStdString() + ";\n";
        }
        else if (v.type == "char") {
            QString str = v.initialValue.toString();
            int charCode = str.isEmpty() ? 0 : str.at(0).unicode();
...
void PetriScene::executeAction(const QString& action) {
    ...
    for (auto& v : variables) {
        ...
        if (v.type == "int") {
            before[v.name] = QString::number(CflatValueAs(val, int));
        }
        else if (v.type == "float")  {
            before[v.name] = QString::number(CflatValueAs(val, float));
        }
        else if (v.type == "char") {
    ...
    std::string wrapped = globalDecls.toStdString() + "void __action() {\n" 
    ...
    for (auto& v : variables) {
        ...
        QString after;
        if (v.type == "int") {
            after = QString::number(CflatValueAs(val, int));
        }
        else if (v.type == "float") {
            after = QString::number(CflatValueAs(val, float));
        }
        else if (v.type == "char") {
            int ascii = CflatValueAs(val, int);
    ...
    for (const QString& name : outputNames) {
        ...
        if (!flag || CflatValueAs(flag, int) == 0) {
            continue;
        }
        ...
        if (floatVal == (float)(int)floatVal) {
            // If no decimal part, show as int
            strVal = QString::number((int)floatVal);
        }
        else {
            // Has decimal, show as float
            strVal = QString::number(floatVal, 'g', 6);
        }
