...
void myMessageOutput(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
    ...
    QMetaObject::invokeMethod(globalLogWidget, "appendPlainText", Qt::QueuedConnection,
...
int main(int argc, char* argv[]) {
    ...
    // Rebuilds the side panel buttons
    rebuildPanel = [=]() {
        ...
        delete panel->layout();
        QList<QWidget*> children = panel->findChildren<QWidget*>(QString(), Qt::FindDirectChildrenOnly);
        ...
        if (scene->currentMode == PetriScene::RunMode) {
            ...
            for (auto& v : scene->variables) {
                ...
                // Update label when any transition fires
    ...
    QObject::connect(openAction, &QAction::triggered, [=]() {
        QString filePath = QFileDialog::getOpenFileName(toolbar, "Open Petri Net", "", FILE_IO_FILETYPES);
    ...
    QObject::connect(saveAction, &QAction::triggered, [=]() {
        QString filePath = QFileDialog::getSaveFileName( toolbar, "Save Petri Net", "", FILE_IO_FILETYPES);
    ...
    for (QAction* action : toolActions) {
        action->setCheckable(true); //Stays "pressed" when clicked
